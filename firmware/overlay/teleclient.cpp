...
// FCM patch: server endpoint comes from the stored config at runtime instead
// of the compile-time SERVER_HOST/SERVER_PORT/SERVER_PATH macros. Defined and
// seeded in telelogger.ino (fcmApplyConfig).
extern char fcmSrvHost[];
extern uint16_t fcmSrvPort;
extern char fcmSrvPath[];
extern bool fcmSrvMethodGet; // HTTPS transmit style: GET query-string vs POST body
...
bool TeleClientUDP::connect(bool quick)
{
  ...
  if (wifi.connected())
  {
    if (quick) return wifi.open(fcmSrvHost, fcmSrvPort);
  }
  else
#endif
  {
  ...
  for (byte attempts = 0; attempts < 3; attempts++) {
    ...
    Serial.print(fcmSrvHost);
    Serial.print(':');
    Serial.print(fcmSrvPort);
    Serial.println(")...");
#if ENABLE_WIFI
    if (wifi.connected())
    {
      if (!wifi.open(fcmSrvHost, fcmSrvPort)) {
...
#endif
    {
      if (!cell.open(fcmSrvHost, fcmSrvPort)) {
        if (!cell.check()) break;
...
bool TeleClientUDP::ping()
{
  ...
  for (byte n = 0; n < 3 && !success; n++) {
    ...
    if (wifi.connected())
    {
      success = wifi.open(fcmSrvHost, fcmSrvPort);
    }
    else
#endif
    {
      success = cell.open(fcmSrvHost, fcmSrvPort);
    }
    if (success) {
...
{
  char path[256];
  snprintf(path, sizeof(path), "%s/notify/%s?EV=%u&SSI=%d&VIN=%s", fcmSrvPath, devid,
...
#endif
  {
    return cell.send(METHOD_GET, fcmSrvHost, fcmSrvPort, path) && cell.receive() && cell.code() == 200;
  }
}
...
bool TeleClientHTTP::transmit(const char* packetBuffer, unsigned int packetSize)
{
...
#endif
  {
    Serial.print("[CELL] ");
    Serial.println(path);
    success = cell.send(METHOD_POST, fcmSrvHost, fcmSrvPort, path, packetBuffer, packetSize);
  }
  len += packetSize;
  }
  if (!success) {
...
#if ENABLE_WIFI
  if (wifi.connected()) success = wifi.open(fcmSrvHost, fcmSrvPort);
#endif
  if (!success) {
