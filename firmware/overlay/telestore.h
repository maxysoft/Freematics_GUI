...
public:
    ...
    virtual void dispatch(const char* buf, byte len);
    // FCM patch: begin() was only declared on the leaf classes; hoisting it as
    // virtual lets the sketch drive SD/SPIFFS through a FileLogger* chosen at
    // runtime from the stored config.
    virtual uint32_t begin() { return 0; }
    virtual ~FileLogger() {}
