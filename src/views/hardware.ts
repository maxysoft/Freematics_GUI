...
export function createHardwareView(
  ...
  return createFormView({
    ...
    onApplied,
    fields: [
      { name: "enable_obd", label: "Enable OBD-II", type: "checkbox", readonly: true, desc: "Read vehicle data over the OBD-II port (RPM, speed, coolant temp, etc.). Set in the firmware build (ENABLE_OBD) — not changeable at runtime." },
      { name: "enable_mems", label: "Enable MEMS (IMU)", type: "checkbox", readonly: true, desc: "Use the on-board motion sensor for orientation and motion/crash detection. Compile-time (ENABLE_MEMS)." },
      { name: "enable_httpd", label: "Enable HTTP Server", type: "checkbox", readonly: true, desc: "Run a small HTTP server for local access over WiFi. Compile-time (ENABLE_HTTPD)." },
      { name: "gnss_mode", label: "GNSS Mode", type: "select", readonly: true, options: GNSS_OPTIONS, desc: "GPS source: Off, Standalone (own GPS chip), or Assisted via the cellular module. Selected at build time (GNSS) — not changeable at runtime." },
      { name: "storage", label: "Storage", type: "select", readonly: true, options: STORAGE_OPTIONS, desc: "Where data is logged locally: none, internal flash (SPIFFS), or SD card. Compile-time (STORAGE)." },
      { name: "board_has_psram", label: "Board has PSRAM", type: "checkbox", readonly: true, help: "Read-only flag from device", desc: "Indicates the ESP32 module has extra PSRAM. Compile-time hardware flag — informational." },
      ...
      { name: "gnss_always_on", label: "GNSS Always On", type: "checkbox", readonly: true, desc: "Keep the GPS powered during sleep for faster fixes, at higher power cost. Compile-time (GNSS_ALWAYS_ON)." },
      { name: "gnss_reset_timeout", label: "GNSS Reset Timeout (s)", type: "number", min: 0, step: 1, desc: "Seconds without a GPS fix before the GPS module is reset to recover. 0 keeps the firmware default." },
