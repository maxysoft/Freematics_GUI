...
export function createServerView(
  ...
  return createFormView({
    ...
    onApplied,
    fields: [
      { name: "server_host", label: "Server Host", type: "text", placeholder: "telemetry.example.com", desc: "Hostname or IP address of the telemetry server the device uploads to (e.g. hub.freematics.com or your own server)." },
      { name: "server_port", label: "Server Port", type: "number", min: 0, max: 65535, step: 1, desc: "Network port on the telemetry server. Freematics Hub uses 8081 (UDP) by default." },
      {
        name: "server_protocol",
        label: "Protocol",
        type: "select",
        options: PROTOCOL_OPTIONS,
        desc: "How data is sent to the server: UDP (lowest overhead, default) or HTTPS GET/POST (encrypted, higher overhead).",
      },
      { name: "server_path", label: "Server Path", type: "text", placeholder: "/api/telemetry", desc: "URL path appended to the host for the HTTPS modes (e.g. /push). Ignored for UDP." },
